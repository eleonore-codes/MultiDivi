...
export function progressHTML(state){
  ...
  let html=`<p class="eyebrow">Für Erwachsene</p><h1>Fortschritt</h1><div class="stats"><div class="stat"><strong>${state.learningDays.length}</strong>Lerntage</div><div class="stat"><strong>${state.successNumber}</strong>Erfolge</div><div class="stat"><strong>${currentStreak(state)}</strong>Tage in Folge</div><div class="stat"><strong>${state.longestStreak}</strong>Längste Serie</div></div><p>Alle sechs Level sind verfügbar.</p><p>Trainierte Level: ${Object.entries(state.levelProgress).filter(([,p])=>p.observations>0).map(([l])=>l).join(', ')||'noch keine'}.</p>`;
  for(let l=1;l<=4;l++){
