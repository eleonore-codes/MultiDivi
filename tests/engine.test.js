...
test('Exactly five successes unlock level 2; weak results never subtract or relock',()=>{
 const s=freshState();for(let i=1;i<=5;i++){awardSuccess(s,report(String(i)),'2026-09-20');awardSuccess(s,report('weak'+i,'a',1,20),'2026-09-20');unlockLevels(s);assert.equal(s.unlockedLevels.includes(2),i===5);}assert.equal(s.level2QualifyingSuccesses,5);s.levelProgress={};unlockLevels(s);assert(s.unlockedLevels.includes(2));
});
test('Later levels require coverage and strategy evidence',()=>{
 const s=freshState();s.levelProgress[1]={successes:8,observations:180};assert.equal(unlockLevels(s).includes(3),false);for(const t of TASKS.slice(0,100))for(let i=0;i<6;i++)observe(s.factMastery,t,true,timing);assert(unlockLevels(s).includes(3));s.unlockedLevels.push(5);s.levelProgress[5]={successes:4,observations:20,fullTasks:10,correctFullTasks:8};assert(unlockLevels(s).includes(6));
});
...
test('3+2 cycle: idempotent finish, unique days, summaries and no third phase',()=>{
 const s=freshState();assert(startSession(s,1,'2026-09-20','test'));for(let i=0;i<12;i++){const w=newWork(TASKS[0]);acceptValue(TASKS[0],w,TASKS[0].answer,timing);recordTask(s,TASKS[0],w,'2026-09-20');}finishPhase(s,'2026-09-20');finishPhase(s,'2026-09-20');assert.equal(s.successNumber,1);assert(startFocus(s));for(let i=0;i<8;i++){const w=newWork(TASKS[1]);acceptValue(TASKS[1],w,TASKS[1].answer,timing);recordTask(s,TASKS[1],w,'2026-09-20');}finishPhase(s,'2026-09-20');assert.equal(s.successNumber,2);assert.equal(s.session.stage,'done');assert.equal(s.learningDays.length,1);assert.equal(s.dailySummaries['2026-09-20'].reports.length,2);assert.equal(startSession(s,1,'2026-09-20'),false);assert(startSession(s,1,'2026-09-21'));assert.equal(durations().a,180000);assert.equal(durations().b,120000);
});
